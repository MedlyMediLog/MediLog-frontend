...
function getStatusIconSrc(status: Exclude<ProductStatus, '섭취 금지'>) {
  return status === '섭취 가능' ? iconSafety : iconWarning
}

export function ProductCard({ layout = 'desktop', item, showStatus = false }: Props) {
  ...
  return (
    <article className="flex w-[302px] flex-col gap-4">
      <div className="relative h-[202px] w-[302px] overflow-hidden rounded-[12px] bg-layer-primary">
        {canShowStatus && (
          <div className="absolute left-[10px] top-[10px] z-10 flex gap-[4px]">
            {renderStatusBadges(item.status as Exclude<ProductStatus, '섭취 금지'>)}
          </div>
        )}

        <Image src={item.image ?? placeholderCard} alt={item.name} fill className="object-cover" />
      </div>
