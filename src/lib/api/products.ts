...
export async function getProducts(
  ...
  const categoryData = MOCK_PRODUCTS[category]

  if (!categoryData) {
    throw new Error(`Invalid category: ${category}`)
  }

  console.log('df', categoryData)
  console.log('target', target)

  const data = target && categoryData[target] ? categoryData[target] : categoryData.default

  if (!data) {
    throw new Error(`No data for category=${category}, target=${target}`)
  }
